...
- 다익스트라 / 플로이드 와샬
  - 다익스트라 알고리즘은 그래프에서 여러개의 노드가 있을 때, 특정한 노드에서 출발하여 다른 노드로 가는 각각의 최단 경로를 구해주는 알고리즘.
  - '음의 간선'이 없을 떄 정상적으로 동작
  - 다익스트라 알고리즘은 기본적으로 그리디 알고리즘으로 분류
  - 매번 가장 비용이 적은 노드를 선택해서 임의의 과정을 반복하기 때문에
